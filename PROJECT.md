...
- [ ] 图片区 220px 宽，三张 grid 均分填满（Vol.002 新增）
- [ ] 话题与游戏研发有关联性（体育占比不超过 2/10）

#### 3c. TikTok 板块
- [ ] 统计指标行（总播放量/视频数/排名方式/采集日期）
- [ ] 方法论框（说明数据来源和排名方式）
- [ ] 结论框（3 列方向总结）
...
#### 3d. YouTube Shorts 板块
- [ ] 结论框（3 列方向总结）
- [ ] 10 条卡片（缩略图 + 链接 + ⤢ 弹窗嵌入播放）
- [ ] 数据总览表（频道/标题/播放量/点赞/日期/类型）
...
#### 3f. 广告板块
- [ ] 方法论框（数据来源 + 人气值/展示估值/热度含义说明）（Vol.002 新增）
- [ ] 结论框（3 列方向总结）
- [ ] 10 条卡片，视频直接播放（autoplay muted loop）+ ⤢ 弹窗放大（Vol.002 新增）
- [ ] 视频区 240px 宽，`object-fit: contain`（Vol.002 新增）
- [ ] 广告视频与游戏名称一一对应
- [ ] 无 HTML 类型广告（应已跳过）

#### 3g. 全局检查
- [ ] 每板块结论框方向与西幻卡牌游戏相关
- [ ] A 级洞察数量合理（通常 10-18 条）
- [ ] Emoji 全部正常渲染（无 ?? 或乱码）
- [ ] 侧栏：「全部期刊」列出所有期数 + 当前期高亮
- [ ] 前一期侧栏已更新（添加新期链接）

#### 3h. 附属文件
- [ ] share-card.html：填入真实内容（图片/信号卡片/数据，非占位符）
- [ ] index.html：新期卡片已添加 + 旧期移除 latest 标记
...
发布后追加：
- [ ] 确认 GitHub Pages 部署成功（等 1-2 分钟后访问线上地址）
- [ ] 可选：提交中间文件归档（JSON 数据 + 组装脚本 + section 文件）
