...
| 🎵 TikTok 热门 | TikTok Explore 美区 | Apify | `APIFY_TOKEN` |
| ▶️ YouTube Shorts | YouTube 频道页 | YouTube Data API | `YOUTUBE_API_KEY` |
...
**Step 4 — YouTube Shorts**
```bash
python3 ~/.claude/skills/youtube-research/scripts/fetch_youtube.py \
  --days 14 --limit 30 --output "$REPORT_DIR/yt_raw.json"
...
| `fetch_ads_with_video.py` | 🔲 Vol.002 前完成 | 同上（待写） |
