...
- ✅ 允许的措辞：「值得关注」「可以参考」「不妨联想到」「是个不错的起点」「成熟参考模板」「通用节奏」「可作为...的候选」「可衍生出」「值得纳入选题池」
- ❌ 禁用的措辞：「必须」「立刻执行」「直接可执行」「硬规则」「现成模板」「碾压」「最优」「最强判断」「不能」「严禁」（反向参考除外）「DRD 必须」
- "本周策划行动" → 改为 "**💡 可联想的方向**"
- "反信号 / 严禁" → 改为 "**⚠ 反向参考**"
...
echo "禁用词扫描:"
grep -E '必须|立刻|严禁|硬规则|现成模板|碾压|最强判断' $OUT | head -5  # 期望 0 行
echo "图片资源存在性:"
